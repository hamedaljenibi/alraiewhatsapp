...
const animalSchema = new Schema<IAnimal>(
  {
    ...
    customBreedName: { type: String, trim: true },

    uniqueAnimalId: { type: String, required: true, unique: true, index: true }, // AN-...
    microChipId1: { type: String, trim: true },
    microChipId2: { type: String, trim: true },
    microChipId3: { type: String, trim: true },
    ...
    purpose: { type: String, enum: ["racing" ,"beauty", "production", "transportation" , "security" , "hunting" , "companionship" ,"research", "therapy", "donor" , "surrogate" ],  default: null  },
...
animalSchema.index(
  { microChipId1: 1 },
  { unique: true, partialFilterExpression: { microChipId1: { $exists: true, $type: "string", $ne: "" } } }
);

animalSchema.index(
  { microChipId2: 1 },
  { unique: true, partialFilterExpression: { microChipId2: { $exists: true, $type: "string", $ne: "" } } }
);

animalSchema.index(
  { microChipId3: 1 },
  { unique: true, partialFilterExpression: { microChipId3: { $exists: true, $type: "string", $ne: "" } } }
);
